...
var tree = d3.layout.tree()
    .size([360, diameter / 2 - 120]) // affects the placement of the right side of the tree
    .separation(function(a, b) { return (a.parent == b.parent ? 1 : 2) / a.depth; });

var diagonal = d3.svg.diagonal.radial()
    .projection(function(d) { return [d.y, d.x / 180 * Math.PI]; }); //changes line layout

var svg = d3.select("#techWheel").append("svg")
    .attr("width", diameter)
    .attr("height", diameter)// affects the center of the wheel. Increase value to lower center.
    .append("g")
    .attr("transform", "translate(" + diameter / 2 + "," + diameter / 2 + ")"); // move entire diagram center
...
d3.json("./assets/scripts/techFamily.json", function(error, root) {
    ...
    var node = svg.selectAll(".node")
        ...
        .enter().append("g")
        .attr("class", "node")
        .attr("transform", function(d) { return "rotate(" + (d.x - 90) + ")translate(" + d.y + ")"; }); // rotates the text wheel
    node.append("circle")
        .attr("r", 4.5); // changes the size of the blue circles
